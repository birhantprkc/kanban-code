...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    open func linefeed(source: Terminal) {
        // Preserve manual selection while output is streaming — only clear
        // when the terminal app has actually requested mouse events.
    ...
    /// OSC 52 from the program: hand it to the delegate, which owns the
    /// pasteboard (LocalProcessTerminalView writes it there).
